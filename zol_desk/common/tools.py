...
class PageTool(object):
    ...
    def ele_clicks(self, driver, xpaths, now_whd, is_bt, is_judge_link, is_mix, pc_bt_xpath, mo_bt_xpath, pc_or_mobile, is_attr, attr, is_back, mv_xp):
        res = []
        try:
    ...
    def judge_jump(self, is_bt, is_judge_link, zx_txt, pg_tt, driver, is_mix, pc_bt_xpath, mo_bt_xpath, now_hd, pc_or_mobile, is_back, mv_xp):
        # 判断页面跳转以及按钮
        if is_bt:
            if is_judge_link:
                judge = self.judgestr(zx_txt, pg_tt)
                if judge:
                    self.my_logger.info('desk.zol页面"{}"跳转正常'.format(zx_txt))
                else:
                    self.my_logger.error('desk.zol页面"{}"跳转异常'.format(zx_txt))
            ...
            if is_judge_link:
                judge = self.judgestr(zx_txt, pg_tt)
                if judge:
                    self.my_logger.info('desk.zol首页"{}"跳转正常'.format(zx_txt))
                else:
                    self.my_logger.error('desk.zol首页"{}"跳转异常'.format(zx_txt))
